...
function isInsideCircle(circle, point) {
  ...
  return circle.radius > Math.sqrt((point.x - x) ** 2 + (point.y - y) ** 2);
}
...
function findFirstSingleChar(str) {
  ...
  let char = null;
  arr.forEach((element) => {
    if (arr.indexOf(element) === arr.lastIndexOf(element) && char === null) {
      char = element;
    }
  });
  return char;
...
function getDigitalRoot(num) {
  let res = num;
  while (res > 9) {
    res = String(res).split('').reduce((a, b) => a + Number(b), 0);
  }
  return res;
}
...
function isBracketsBalanced(str) {
  ...
  while (i < str.length) {
    ...
    if (brackets[char]) {
      ...
      if (result.length === 0 || brackets[result.pop()] !== char) {
        return false;
      }
    }
    i += 1;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathArr[0].length; i += 1) {
    const part = pathArr[0][i];
    if (pathArr.every((path) => path[i] === part)) {
      parts.push(part);
    } else {
      break;
    }
